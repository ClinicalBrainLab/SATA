function Mean_Std_of_Lobes = SATA_CBL_Extract_Plot_Mean_STD_Cortical_Roast(CD_array,tal_in,brain_region)
%SATA_CBL_Extract_Plot_Mean_STD_Cortical_Roast(CD_array,tal_in,brain_region)
%
...
for k =1:l
        if contains(cortical(k),"Left")
            temp = split(cortical(k));
            cortical(k) = "Right";
            for t=2:length(temp)
            cortical(k)=cortical(k)+" "+temp(t);
            end

        elseif contains(cortical(k),"Right")
            temp = split(cortical(k));
            cortical(k) = "Left";
            for t=2:length(temp)
            cortical(k)=cortical(k)+" "+temp(t);
